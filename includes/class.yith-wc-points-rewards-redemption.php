...
if ( ! class_exists( 'YITH_WC_Points_Rewards_Redemption' ) ) {
	...
	class YITH_WC_Points_Rewards_Redemption {
		...
		public function __construct() {
			...
			add_action( 'woocommerce_remove_cart_item', array( $this, 'update_discount' ) );
		 ...
		 * @param string   $status_from Status.
		 *
		 * @return void
		 */
		...
		public function calculate_rewards_discount( $discount_amount = 0.0 ) {
			...
			foreach ( $items as $item => $values ) {
				...
				$product_discount = (float) $this->calculate_product_max_discounts( $product_id, $item_price );
				if ( $product_discount != 0 ) {
					$this->max_discount += $product_discount * (float) $values['quantity'];
				}
			}
		...
		public function calculate_product_max_discounts( $product_id, $price = 0 ) {
			...
			return apply_filters( 'ywpar_calculate_product_max_discounts', $max_discount, $product );
		}
